...
impl PlotUi {
    ...
    // grom
    pub fn hray(&mut self, mut hray: HRay) {
        if hray.stroke.color == Color32::TRANSPARENT {
            hray.stroke.color = self.auto_color();
        }
        self.items.push(Box::new(hray));
    }

    // grom
    pub fn linked_y_hray(&mut self, mut hray: LinkedYHRay) {
        if hray.stroke.color == Color32::TRANSPARENT {
            hray.stroke.color = self.auto_color();
        }
        self.items.push(Box::new(hray));
    ...
    // grom
    pub fn linked_y_text(&mut self, text: LinkedYText) {
        if text.text.is_empty() {
            return;
        };
    ...
    // grom
    pub fn linked_y_polygon(&mut self, mut polygon: LinkedYPolygon) {
        if polygon.series.is_empty() {
            return;
        };
