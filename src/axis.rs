...
pub(super) struct AxisWidget {
    ...
    // grom
    pub(super) highlights: Vec<(f64, Color32)>,
}
...
impl AxisWidget {
    ...
    pub fn ui(self, ui: &mut Ui, axis: Axis) -> (Response, f32) {
        ...
        for (value, color) in self.highlights {
            ...
            let galley = WidgetText::RichText(text)
                .into_galley(ui, Some(false), f32::INFINITY, font_id.clone());

            let pos = {
                let projected_point = super::PlotPoint::new(0.0, value);
                let center_y = transform.position_from_point(&projected_point).y;
                let x = self.rect.min.x;
                Pos2::new(x, center_y - galley.size().y / 2.0)
            };

            let text_color = super::color_from_strength(ui, 1.0);
            ui.painter().add(TextShape::new(pos, galley, text_color));
        }
